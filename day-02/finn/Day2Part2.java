...
public class Day2Part2 {
    ...
    public static void main(String[] args) {
        ...
        List<List<Integer>> reports = input.stream().map(line -> Arrays.stream(line.split(" ")).map(Integer::valueOf).toList()).toList();

        final long amountOfSaveReports = reports.stream().filter(Day2Part2::isSafeFor2).count();

        System.out.println(amountOfSaveReports);
    }

    private static boolean isSafeFor2(final List<Integer> report) {
        if (isSafe(report, null, 0)) {
            return true;
        }
    ...
    private static boolean isSafe(final List<Integer> report, Boolean decresing, int index) {
        int value1 = report.get(index);
        int value2 = report.get(++index);

        if (decresing == null) {
            decresing = value1 > value2;
        }

        boolean result = safePair(value1, value2, decresing);

        if (index + 1 < report.size()) {
            return result && isSafe(report, decresing, index);
        }

        return result;
    }

    private static boolean safePair(final int value1, final int value2, final boolean decresing) {
        int difference = Math.abs(value1 - value2);

        if (decresing && value1 < value2) {
            return false;
        } else if (!decresing && value1 > value2) {
            return false;
        }
